...
public abstract class SingleParamFramework<P1, R> implements CodeFramework {
    ...
    protected void methodTemplate() {
        ...
        for (int i = 0; i < testTimes; i++) {
            ...
            if (!compareResult(solutionResult, checkResult)) {
                success = false;
                System.out.printf(
                        "测试失败，\n失败用例：「%s」,\n对数器结果:「%s」，\n题解结果:「%s」", param1, checkResult, solutionResult);
                break;
            }
        }
        if (success) {
    ...
    protected boolean compareResult(R solutionResult, R checkResult) {
        return Objects.equals(solutionResult, checkResult);
    }
     ...
     * @return 计算结果
     */
    protected abstract R solution(P1 param1);
     ...
     * @return 计算结果
     */
    protected abstract R check(P1 param1);
}
