...
public class JolResearch {
  ...
  public void testJolLinkedHashMap() throws InterruptedException {
    ...
    System.out.println("map 总内存占用：" + GraphLayout.parseInstance(linkedMap).totalSize());
    System.out.println("array 总内存占用：" + GraphLayout.parseInstance(arrayList).totalSize());
    System.out.println("linked 总内存占用：" + GraphLayout.parseInstance(linkedList).totalSize());
    System.out.println("key内存占用：" + keyBytes);
